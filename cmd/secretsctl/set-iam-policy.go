...
func init() {
	SecretManagerServiceCmd.AddCommand(SetIamPolicyCmd)
...
var SetIamPolicyCmd = &cobra.Command{
	Use:   "set-iam-policy",
	Short: "Sets the access control policy on the specified...",
	Long:  "Sets the access control policy on the specified secret. Replaces any  existing policy.   Permissions on...",
	...
	RunE: func(cmd *cobra.Command, args []string) (err error) {
		...
		if Verbose {
			printVerboseInput("SecretManager", "SetIamPolicy", &SetIamPolicyInput)
		}
		resp, err := SecretManagerClient.SetIamPolicy(ctx, &SetIamPolicyInput)
